...
app.post('/wheel/topups/apply', async (req, res) => {
  try {
    ...
    const source = ['manual', 'ton', 'admin'].includes(req.body?.source)
      ? req.body.source
      : 'manual';
...
app.get('/me/entitlements', async (req, res) => {
  try {
    ...
    let subscriptionDay = 0;
    if (activeEntitlement && activeEntitlement.status === 'active') {
      const start = new Date(activeEntitlement.startsAt);
      const now = new Date();
      const diffTime = now.getTime() - start.getTime();
      subscriptionDay = Math.floor(diffTime / (1000 * 60 * 60 * 24)) + 1;
    }
